...
router.post('/querystatics', function (req, res, next) {
  ...
  var data = {}
    ...
    Promise.all([
      statics.QueryWorkPie(start,end), 
      statics.QueryWork(start,end,'day,mon',null,null),
                                                                                                
      statics.QueryOrderPie(start,end,'TaskTypeName,TaskStatusDescription,OrderID',{'TaskTypeName':'成品入库任务,手动送料任务'}),     //总计饼图     
      statics.QueryOrder(start,end,'TaskTypeName','day,mon',null),            // 返回日走线图  月走线图      
      statics.QueryOrder(start,end,'TaskStatusDescription','day,mon',null),             
      statics.QueryStaticsPie(start,end,'TaskTypeName,MaterialID,OrderID'),  
      statics.QueryStatics(start,end,'MaterialID','day,mon',null),             // 返回日走线图  月走线图   
      statics.QueryCharCount(start,end)
    ])
      .then(data=>{
        ...
        if(len!=0){                                               //订单统计返回
          resJson.ReturnData.push( 
              ...
              title:'订单统计',
              type:'OrderStyle',
              pie:data[2],                                         //[任务类别,任务状态,工单id]
           
              line:[{dayline:data[3][0],
                     monline:data[3][1]},
                    {dayline:data[4][0],
                     monline:data[4][1] 
              }  ] 
