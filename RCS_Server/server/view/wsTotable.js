...
import dbHelper from '../dbHelper/DBConnection'
  ...
  //更新taskinfo数据到数据库：在socket端调用
   var UpdateTaskinfo=function (table,obj,id){ 
       let ColArr = ['TaskID','AGVID','AGVIP','StartPlaceDescription','EndPlaceDescription'
         ...
         ,'TokenStartTime','TokenUseTime','TokenMaxTime','TokenOffset','Sender','MaterialNum','MaterialID','Number'
         ,'Unit','LabelNumber','LineID','OrderID','CancelCurrentTask']
   ...
   var InsertTaskinfo=function (table,obj){
     ...
     let ColArr = ['TaskID','AGVID','AGVIP','StartPlaceDescription','EndPlaceDescription'
       ,'TaskTypeName','TaskType','TaskStatus','TaskStatusDescription','StartTime','FinishTime','SetTime','TokenFinishTime'
       ,'TokenStartTime','TokenUseTime','TokenMaxTime','TokenOffset','Sender','MaterialNum','MaterialID','Number'
       ,'Unit','LabelNumber','LineID','OrderID','CancelCurrentTask', 'MaterialIDs']
     let CollStr = new Array();
     let valStr = new Array()
           ColArr.forEach(item => {                   
              CollStr+=item+','
  ...
  var InsertWorktime=function (table,obj){ 
   ...
   let ColArr = ['AGVID','AGVIP','AbNormalWorkEndTime','AbNormalWorkStartTime','AbNormalWorkTotal'
   ,'NormalWorkEndTime','NormalWorkStartTime','NormalWorkTotal','WorkDate']
   let CollStr = new Array();
   let valStr = new Array()
     ColArr.forEach(item => {                   
        CollStr+=item+','
